...
def fuv2img(fuv, coorW=1024, floorW=1024, floorH=512):
    ...
    floor_plane_x, floor_plane_y = (
        -(floor_plane_y - floorH / 2),
        floor_plane_x - floorW / 2,
    )
    floor_plane_coridx = (
        np.arctan2(floor_plane_y, floor_plane_x) / (2 * PI) + 0.5
    ) * coorW - 0.5
...
def get_rot_rad(
    init_coorx, coory, z=50, coorW=1024, coorH=512, floorW=1024, floorH=512, tol=5
):
...
def gen_ww_general(init_coorx, xy, gpid, tol):
    ...
    while True:
        ...
        if num_tbd_neighbor == 1:
            if (
                not xy_cor[p_idx]["tbd"]
                and xy_cor[p_idx]["type"] == xy_cor[tbd]["type"]
            ) or (
                not xy_cor[n_idx]["tbd"]
                and xy_cor[n_idx]["type"] == xy_cor[tbd]["type"]
            ):
                ...
                if xy_cor[tbd]["score"] >= -1:
                    ...
                    if not xy_cor[p_idx]["tbd"]:
                        ...
                        if xy_cor[p_idx]["type"] == 0:
                            new_val = np_x_u_solve_y(
                                xy_cor[p_idx]["val"], xy_cor[p_idx]["u1"]
                            )
                            new_type = 1
                        else:
                            new_val = np_y_u_solve_x(
                                xy_cor[p_idx]["val"], xy_cor[p_idx]["u1"]
                            )
                            new_type = 0
                    else:
                        ...
                        if xy_cor[n_idx]["type"] == 0:
                            new_val = np_x_u_solve_y(
                                xy_cor[n_idx]["val"], xy_cor[n_idx]["u0"]
                            )
                            new_type = 1
                        else:
                            new_val = np_y_u_solve_x(
        ...
        if xy_cor[p_idx]["type"] == xy_cor[n_idx]["type"]:
            ...
            if xy_cor[tbd]["type"] == xy_cor[p_idx]["type"]:
                ...
                xy_cor[tbd]["val"] = xy[
                    gpid == xy_cor[tbd]["gpid"], xy_cor[tbd]["type"]
                ].mean()
        else:
...
def cor_2_1d_unsorted(cor, H, W):
    ...
    for i in range(n_cor // 2):
        ...
        # Handle wrapping: xys[:, 0] might be outside [0, W-1] or have jumps?
        # pano_connect_points returns coorxs = (np.arange(pstart, pend + 1) % w)
        # So x is already modulo W.
